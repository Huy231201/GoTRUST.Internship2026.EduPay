...
import { downloadClassTemplate } from "@/lib/api/class";
...
export default function ImportClassView({ onBack }: Props) {
    const [step, setStep] = useState(1);
    const [file, setFile] = useState<File | null>(null);
    const [result, setResult] = useState<any>(null);

    const { branchId, schoolYearId } = useAppFilterStore();
    const { importClassesFromFile } = useClassStore();
    ...
    const handleDownloadTemplate = async () => {
        ...
        try {
            const blob = await downloadClassTemplate(branchId, schoolYearId);
            const url = window.URL.createObjectURL(blob);

            const link = document.createElement("a");
            link.href = url;
            link.download = "class-import-template.xlsx";
    ...
    const handleNext = async () => {
        if (step === 1) {
            ...
            try {
                const res = await importClassesFromFile(file!);
                setResult(res);
                setStep(3);
            } catch (err) {
    ...
    const handleBack = () => {
        // ✅ Step 3 → reset về step 1 (import lại)
        if (step === 3) {
            setStep(1);
            setFile(null);
            setResult(null);
    ...
    return (
        <div className="flex flex-col gap-5">
            <PageHeader
                ...
                subtitle="Thực hiện nhập danh sách lớp học từ tệp "
            />
                    ...
                    <CardContent className="flex flex-col pb-2">
                        ...
                        <div className="flex items-center justify-center pt-5 gap-3">
                            <div className="flex gap-2 items-center">
                                <div className={`flex w-8 h-8 rounded-full items-center justify-center text-xs font-medium ${getStepStyle(1)}`}>
                                    1
                                </div>
                            ...
                            <div className="flex gap-2 items-center">
                                <div className={`flex w-8 h-8 rounded-full items-center justify-center text-xs font-medium ${getStepStyle(2)}`}>
                                    2
                                </div>
                            ...
                            <div className="flex gap-2 items-center">
                                <div className={`flex w-8 h-8 rounded-full items-center justify-center text-xs font-medium ${getStepStyle(3)}`}>
                                    3
                                </div>
                        ...
                        {step === 1 && (
                            ...
                            ${isDragActive ? "border-blue-600 bg-blue-100" : "border-blue-500 bg-blue-50/30"}
                            `}
                                style={{
                            ...
                            <div className="flex gap-3 ">
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    <p className="text-sm text-blue-600 font-medium">
                                        Sử dụng mẫu tệp CSV để đảm bảo dữ liệu được nhập chính xác và tránh lỗi khi xử lý
                                    </p>
